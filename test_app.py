...
from layouts import serve_roc_curve, serve_pie_confusion_matrix, no_with_comma, no_to_percentage, home_tab_layout, churn_data_tab, churn_model_tab_layout, sentiment_tab
import pytest
...
# test tab layouts output to
def test_tab_pages():
    assert sentiment_tab() is not None
    assert home_tab_layout() is not None
    assert churn_data_tab() is not None
    assert churn_model_tab_layout() is not None
